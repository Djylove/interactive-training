"""Muon GPT pretraining driven from the Aim frontend.

Same training loop as `examples.muon_gpt`, with Aim UI control (knobs, pause/resume,
pre-flight) wired in. Same model size as the original; 1000 steps/round for a
faster 1xH100 demo.

Run (inside apptainer, after `source init.sh`):
    python -m examples.muon_gpt_frontend --max-rounds 4
    python -m examples.muon_gpt_frontend --preflight --max-rounds 4
    python -m examples.muon_gpt_frontend --no-agent --max-rounds 1
"""
...
def main(args):
    ...
    run_dir, run_id = setup_logs("muon_gpt_frontend")
    ...
    session = TrainingSession(
        goal="val_loss", memory=memory_path, agent=agent,
        max_rounds=args.max_rounds, context=build_context(args), seed=args.seed,
        watch_metrics=["val_loss", "weight_rms", "grad_norm"],
        frontend={"repo": aim_repo, "experiment": f"muon_gpt_frontend-{run_id}",
    ...
    def train_round(session, ctx):
        ...
        for it in range(args.max_steps):
            ...
            if (it + 1) % 10 == 0 or it + 1 == args.max_steps:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Frontend-controlled Muon GPT pretraining")
    ...
    p.add_argument("--max-steps", type=int, default=1000, help="training steps per round")
    ...
    p.add_argument("--aim-repo", default=None,
                   help="Aim repo path (defaults to logs/muon_gpt_frontend/<run-id>/aim)")
