...
#SBATCH --job-name=layerwise_lr_gpt
...
CMD="cd $PROJ_DIR && set -e && source init.sh && python -m examples.layerwise_lr_gpt \
    --max-rounds 10 \
    --seed $SEED \
    --wandb-experiment layerwise_lr_gpt_seed$SEED \
    --output-dir logs/layerwise_lr_gpt/seed$SEED/layerwise_lr_gpt \
    --memory-path logs/layerwise_lr_gpt/seed$SEED/layerwise_lr_gpt_memory.jsonl \
    2>&1"
